...
class Tracker:
    ...
    def interpolate_ball_positions(self, ball_positions):
        positions = [frame.get(1, {}).get("bbox", []) for frame in ball_positions]
        dataframe = pd.DataFrame(positions, columns=["x1", "y1", "x2", "y2"])
        dataframe = dataframe.interpolate().bfill()
        return [{1: {"bbox": bbox}} for bbox in dataframe.to_numpy().tolist()]
    ...
    def get_object_tracks(self, frames,read_from_stub=False,stub_path=None):
        ...
        for frame_num, detection in enumerate(detections):
            ...
            for object_ind, class_id in enumerate(detections_supervision.class_id):
                if class_id == goalkeeper_class_id and player_class_id is not None:
                    detections_supervision.class_id[object_ind] = player_class_id
        ...
        if stub_path is not None:
            stub_directory = os.path.dirname(stub_path)
            if stub_directory:
    ...
    def draw_team_ball_control(self, frame, frame_num, team_ball_control):
        ...
        control_until_frame = team_ball_control[:frame_num + 1]
        team_1_frames = (control_until_frame == 1).sum()
        team_2_frames = (control_until_frame == 2).sum()
        total_frames = team_1_frames + team_2_frames
        if total_frames == 0:
            return frame

        team_1 = team_1_frames / total_frames * 100
        team_2 = team_2_frames / total_frames * 100
        cv2.putText(frame, f"Team 1 Ball Control: {team_1:.2f}%", (1400, 900),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 3)
        cv2.putText(frame, f"Team 2 Ball Control: {team_2:.2f}%", (1400, 950),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 3)
        return frame
